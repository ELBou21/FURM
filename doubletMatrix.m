function [A] = doubletMatrix(trackerPts, forcingPts, epsilon, mu)
% Create the 3m x 3n regularized doublet matrix to calculate the velocity at m
...
for p = 1:m
    for q=1:n
        ...
        R = (r.^2  + epsilon.^2).^(1/2);
        SD = zeros(3,3);
        %Building the 3x3 doublet matrix for each pair of tracking/forcing pts
        for i = 1:3
            for j = 1:3
                SD(i,j) = (2*xhat(3)*(1-2*(i==3)))*(xhat(j)*(i==3)/R.^3 - xhat(i)*(3==j)/R.^3 - xhat(3)*(i==j)/R.^3 + 3*epsilon.^2*xhat(j)*(i==3)/R.^5 + 3*xhat(3)*xhat(i)*xhat(j)/R.^5); 
            end
        end
        % Inserting the 3x3 matrices into the combined 3Mx3N matrix where
        % M = # of tracking pts and N = # of forcing pts
        A(3*p-2:3*p,3*q-2:3*q) = (1/(8*pi*mu))*SD;
    end
end
end